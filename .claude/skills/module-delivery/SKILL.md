...
description: >
  Hify 项目模块交付流程。按照 咨询梳理→Schema→Entity+Mapper→DTO→Service→Controller→前端对接→完整验收 的
  固定节奏交付新模块，每步有明确产出物和验证方式，关键决策点等待用户确认。
  当用户提到"新模块"、"交付XX模块"、"开发XX功能"、"实现XX管理"、"给XX加CRUD"、
  "开始写XX"、"接下来做XX模块"时使用此技能。
  也适用于对已有模块补充功能（如"给provider加健康检查"、"加个连通性测试接口"），
  此时跳过已完成的阶段，从对应阶段切入。
---

# Hify 模块交付流程

按模块逐个交付，每个模块遵循固定节奏。完成后端 MVC 的每一层 **立即编译或 curl 验证**，不攒到最后。

## 总览

| Step | 目标 | 产出物 | 验证 | 确认点 |
|------|------|--------|------|--------|
| 1. 咨询与设计 | 对齐需求边界和数据模型 | 候选方案对比表 + 数据模型草稿 + 接口清单 | 用户口头确认 | ✋ 方案和模型确认后再写代码 |
| 2. Schema | 数据库 DDL 与设计对齐 | schema.sql + schema-h2.sql | H2 启动验证表创建 | — |
| 3. Entity+Mapper | ORM 层对齐 | Entity + Mapper 接口 | `mvn clean install` | — |
| 4. DTO | 定义请求/响应，隔离实体 | CreateRequest / UpdateRequest / QueryRequest / DetailResponse | `mvn clean install` | — |
| 5. Service | 核心业务实现 | Service 接口 + ServiceImpl | `mvn clean install` | — |
| 6. Controller | 暴露 REST 接口 | Controller | curl 逐接口验证 | ✋ 所有 curl 返回预期后再进前端 |
| 7. 前端 API | 封装后端接口，定义 TS 类型 | `api/{module}.ts` | 编译通过 | — |
| 8. 前端页面 | 替换 mock，接入真实 API | 更新 Vue 页面组件 | 浏览器 DevTools → Network 确认请求正常 | ✋ 页面交互细节等确认 |

> **节奏**：每步完成后立即验证，不攒到最后。Step 3-4 可一次写完再编译。Step 5 最重，按方法逐个实现。
