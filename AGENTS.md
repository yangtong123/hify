...
| 模块 | 说明 |
|------|------|
| 提供商管理 (provider) | 管理 OpenAI / Codex / Gemini / Ollama 等 LLM 提供商配置，支持连通性测试 |
...
com.hify
├── common/
│   ├── config/          # 全局 Spring Bean 配置（MybatisPlusConfig, JacksonConfig, AsyncConfig, ThreadPoolConfig）
│   ├── exception/       # 全局异常定义（BizException, ErrorCode 枚举, GlobalExceptionHandler）
...
判断一个 `@Configuration` 类应该放 `common/config/` 还是 `modules/{module}/infra/config/`：
...
测试代码放在 `src/test/java` 下，包结构镜像 `main` 的包结构：

```
src/test/java/com/hify/
├── common/
│   └── util/                # JsonUtilTest, DateUtilTest
└── modules/
    └── {module}/
        ├── domain/          # Service 单元测试（Mock 外部依赖）
        ├── infra/           # Mapper 集成测试、RepositoryImpl 测试
        └── web/             # Controller 集成测试（MockMvc）
```

- 测试类命名：`{被测类名}Test`（如 `AgentServiceImplTest`）
- 单元测试放在与被测类相同的包路径下
- `common/` 工具类测试放 `src/test/java/com/hify/common/` 对应子包
